...
async function classifyJiefangText() {
  try {
    console.log('开始分类一汽解放PDF的OCR文本...\n');

    // 读取OCR提取的原始数据
    const dataPath = path.join(__dirname, 'jiefang-all-text-data.json');
    ...
    // 保存分类后的数据
    const outputPath = path.join(__dirname, 'jiefang-classified-data.json');
    fs.writeFileSync(outputPath, JSON.stringify(classifiedData, null, 2), 'utf-8');

    console.log('\n\n====== 分类完成 ======');
    console.log(`输出文件: ${outputPath}`);
    console.log(`\n分类统计:`);
    console.log(`  表格文本 (元件标题): ${tableTexts.length} 个片段`);
    console.log(`  说明文本 (元件描述): ${descriptionTexts.length} 个片段`);
    console.log(`  电路图文本 (普通文本): ${circuitTexts.length} 个片段`);

    // 显示每页的分类情况
    console.log('\n====== 各页分类详情 ======');
    for (let i = 1; i <= rawData.totalPages; i++) {
      const tableCount = tableTexts.filter(t => t.pageNumber === i).reduce((sum, t) => sum + t.lineCount, 0);
      const descCount = descriptionTexts.filter(t => t.pageNumber === i).reduce((sum, t) => sum + t.lineCount, 0);
      const circuitCount = circuitTexts.filter(t => t.pageNumber === i).reduce((sum, t) => sum + t.lineCount, 0);

      console.log(`第 ${i} 页: 表格 ${tableCount} 行, 说明 ${descCount} 行, 电路图 ${circuitCount} 行`);
    }
