...
async function extractShanqiPDF() {
  try {
    console.log('开始处理陕汽轩德翼3整车电路图PDF (23页)...\n');
    ...
    for (let i = 0; i < pngPages.length; i++) {
      ...
      try {
        ...
        // 每5页保存一次中间结果,避免内存溢出
        if (pageNumber % 5 === 0 || pageNumber === pngPages.length) {
          const shanqiData = {
          ...
          console.log(`  ✓ 已保存中间结果 (${pageNumber}/${pngPages.length})`);
        }
    ...
    shanqiData.allPageTexts = allPageTexts;
