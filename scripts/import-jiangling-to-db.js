  ...
  try {
    ...
    try {
      ...
      for (const [pageNumber, pageData] of pageDataMap.entries()) {
        ...
        // 无论页面是否有内容，都要插入（包括空白页）
        // 这样确保数据库页码与PDF实际页码一致
        await run(
          'INSERT OR REPLACE INTO document_pages (document_id, page_number, page_text, component_names) VALUES (?, ?, ?, ?)',
          [data.documentId, pageNumber, pageText.trim().length > 0 ? pageText : '[TEXT][/TEXT]', JSON.stringify(componentNames)]
        );
        insertedCount++;

        if (pageNumber % 50 === 0 || pageNumber === data.totalPages) {
