...
async function extractJiefangPDF() {
  try {
    console.log('开始处理一汽解放J6L整车线束图PDF (13页)...\n');

    const pdfPath = path.resolve(__dirname, '../public/pdfs/jiefang_wiring.pdf');
    const outputDirName = 'jiefang-ocr-output';
    ...
    jiefangData.allPageTexts = allPageTexts;

    // 保存完整数据
    const dataPath = path.join(__dirname, 'jiefang-all-text-data.json');
    fs.writeFileSync(dataPath, JSON.stringify(jiefangData, null, 2), 'utf-8');
