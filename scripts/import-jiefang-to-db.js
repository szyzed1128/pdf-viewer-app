...
async function importJiefangToDatabase() {
  try {
    console.log('开始导入一汽解放PDF数据到数据库...\n');

    // 读取分类后的数据
    const dataPath = path.join(__dirname, 'jiefang-classified-data.json');
    ...
    try {
      ...
      const deleteResult = await run('DELETE FROM document_pages WHERE document_id = ?', [data.documentId]);
      ...
      for (const [pageNumber, pageData] of pageDataMap.entries()) {
        ...
        console.log(`  ✓ 第${pageNumber}页已插入 (标题:${pageData.titleTexts.length}, 说明:${pageData.descriptionTexts.length}, 电路图:${pageData.circuitTexts.length})`);
      }
